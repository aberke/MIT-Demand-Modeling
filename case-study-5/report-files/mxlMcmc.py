...
from mxl import corrcov, prepareData, mvnlpdf, probMxl
...
def next_zeta(paramRnd, Omega, nRnd, nInd):
    zeta = paramRnd.mean(axis = 0) + np.ndarray.flatten(np.linalg.cholesky(Omega) @ np.random.randn(nRnd, 1)) / np.sqrt(nInd)
    return zeta
...
if __name__ == "__main__":
    ...
    xRnd = -np.hstack((cost, tt)) #np.zeros((0,0)) #-np.hstack((cost, he, tt))
...
def test(results,
    ...
    if nRnd > 0: postMean_chOmega = np.linalg.cholesky(results['postMean_Omega'])      
                
    for i in np.arange(simDraws_star):
        if nRnd > 0:
            paramRnd = results['postMean_zeta'] + (postMean_chOmega @ np.random.randn(xRnd.shape[1], nInd)).T
    ...
    results = {'unconditionalLogLik': logLik, 'conditionalLogLik':CondLogLik, 
			   'unconditionalChosenProb':np.mean(pSim), 'conditionalChosenProb':np.mean(pSimCond)}
